...
int main(int argc, char* argv[])
{
	...
	void* argtable[] = { show_help, target_arg, input_files, expect_file, actual_file, fail_opt, verbose, quiet, end };
	...
	bfindreplace(ldargs, bfromcstr("testld"), bfromcstr("dtld"), 0);
	...
	if (result != 0 && fail_opt->count == 0)
	{
		...
		printd(LEVEL_ERROR, "expected success but linker returned non-zero exit code (%i).", result);
		return 1;
	}
	else if (result == 0 && fail_opt->count >= 1)
	{
		// Linker returned zero when failure was expected.
		printd(LEVEL_ERROR, "expected failure but linker returned zero exit code.");
		return 1;
	}
	else if (result != 0 && fail_opt->count >= 1)
	{
		...
		if (actual != NULL)
		{
			printd(LEVEL_ERROR, "expected failure but actual output file exists.");
			fclose(actual);
			return 1;
		}
		return 0;
	...
	if (expect == NULL)
	{
		...
		printd(LEVEL_ERROR, "path to expect file does not exist.");
		return 1;
	}
	...
	if (actual == NULL)
	{
		// The expect file was not provided.
		fclose(expect);
		printd(LEVEL_ERROR, "expected data but actual output file does not exist after running linker.");
		return 1;
	}

	// Now compare raw bytes.
	while (!feof(actual) && !feof(expect))
		if (fgetc(actual) != fgetc(expect))
		{
			fclose(actual);
			fclose(expect);
			printd(LEVEL_ERROR, "actual output differs from expected output in content.");
			return 1;
		}
	...
	if (ftell(actual) != ftell(expect))
	{
		fclose(actual);
		fclose(expect);
		printd(LEVEL_ERROR, "actual output differs from expected output in length.");
		return 1;
	}
