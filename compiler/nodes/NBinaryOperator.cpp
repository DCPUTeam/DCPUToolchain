...
NType& NBinaryOperator::getExpressionType(AsmGenerator& context)
{
	// A binary operator has the type of each of it's expressions.
	NType lType = this->lhs.getExpressionType(context);
	NType rType = this->rhs.getExpressionType(context);
	if (lType != rType)
