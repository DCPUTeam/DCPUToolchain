...
ERR_PP_ASM_NO_ENDIF_TO_IF                            pp-asm-no-endif-to-if                  "no matching .ENDIF to .IF near %s"
ERR_PP_ASM_ENDIF_NO_IF                               pp-asm-endif-no-if                     "free occuring .ENDIF without a matching .IF near %s"
ERR_PP_ASM_ELSE_NO_IF                                pp-asm-else-no-if                      "free occuring .ELSE without a matching .IF near %s"
...
ERR_PP_C_NO_ENDIF_TO_IF                              pp-c-no-endif-to-if                    "no matching #endif to #if near %s"
ERR_PP_C_ENDIF_NO_IF                                 pp-c-endif-no-if                       "free occuring #endif without a matching #if near %s"
ERR_PP_C_ELSE_NO_IF                                  pp-c-else-no-if                        "free occuring #else without a matching #if near %s"
