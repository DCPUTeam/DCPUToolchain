...
char_t* ppimpl_get_input(state_t* state)
{
    ...
    if (list_size(&state->cached_input) > 0)
        return list_extract_at(&state->cached_input, 0);
    else
    {
        char_t c = state->input();
        list_append(&state->all_gc, &c);
        return list_get_at(&state->all_gc, list_size(&state->all_gc) - 1);
    }
}
...
{
    if (list_size(&state->cached_output) > 0)
        return ((char_t*)list_get_at(&state->cached_output, list_size(&state->cached_output) - 1))->line;
    else
        return 0;
}
...
{
    if (list_size(&state->cached_output) > 0)
        return ((char_t*)list_get_at(&state->cached_output, list_size(&state->cached_output) - 1))->file;
    else
        return NULL;
}
...
{
    bstring b;
    if (ppimpl_get_current_filename(state) == NULL)
        return bformat("unknown location");
    b = bformat("line %u of '%s'", ppimpl_get_current_line(state), ppimpl_get_current_filename(state)->data);
    return b->data;
}
...
///
/// Prints a formatted string to the output cache of the current state.
///
...
bool ppimpl_match_test(state_t* state, match_t* match)
{
    ...
    for (i = 0; i < (size_t)blength(match->text.ref); i++)
    {
        ...
        {
            if (((char_t*)list_get_at(&state->cached_output, list_size(&state->cached_output) - i - 1))->value !=
                    (char)match->text.ref->data[blength(match->text.ref) - i - 1])
                return false;
        }
        else
        {
            if (tolower(((char_t*)list_get_at(&state->cached_output, list_size(&state->cached_output) - i - 1))->value) !=
    ...
    if (match->line_start_only)
    {
        ...
        while (j >= 0)
        {
            switch (((char_t*)list_get_at(&state->cached_output, j))->value)
            {
                case ' ':
                case '\t':
                    // Permitted; continue.
    ...
    if (match->identifier_only)
    {
        ...
        if (has_next)
        {
            ...
            list_insert_at(&state->cached_input, next, 0);
        }
        if (has_previous)
            previous = list_get_at(&state->cached_output, list_size(&state->cached_output) - blength(match->text.ref) - 1);
        ...
        if (has_next && has_previous)
            is_identifier = ppimpl_isolates(next->value, false) && ppimpl_isolates(previous->value, true);
        else if (has_next)
            is_identifier = ppimpl_isolates(next->value, false);
        else if (has_previous)
            is_identifier = ppimpl_isolates(previous->value, true);
        else
            is_identifier = true;
...
void ppimpl_track_strings(state_t* state)
{
    ...
    current = list_get_at(&state->cached_output, list_size(&state->cached_output) - 1);
    ...
    if (has_previous)
        previous = list_get_at(&state->cached_output, list_size(&state->cached_output) - 2);
    ...
    else if (state->in_single_string)
    {
        ...
        switch (current->value)
        {
            case '\'':
                ...
                // character.
                if (!has_previous || (has_previous && previous->value != '\\'))
...
void ppimpl_process(state_t* state)
{
    ...
    while (ppimpl_has_input(state))
    {
        ...
        list_append(&state->cached_output, c);
    ...
    while (list_size(&state->cached_output) > 0)
        state->output(*(char_t*)list_extract_at(&state->cached_output, 0));
}
...
///
/// @brief Returns the size of char_t elements.
///
static size_t list_meter_char_t(const void* el)
{
    return sizeof(char_t);
}

///
/// @brief Compares the value of two char_t elements.
///
static int list_comparator_char_t(const void* a, const void* b)
{
    const char_t* ac = (const char_t*)a;
    const char_t* bc = (const char_t*)b;
...
void ppimpl(freed_bstring filename, int line, freed_bstring lang, has_t has_input, pop_t input, push_t output)
{
    ...
    list_attributes_comparator(&state.all_gc, list_comparator_char_t);
    ...
    list_attributes_copy(&state.all_gc, list_meter_char_t, true);
    ...
    if (biseqcstrcaseless(lang.ref, "asm"))
    {
        ...
        ppimpl_asm_track_register(&state);
